...
class CollectionPage:
    def __init__(self):
        ...
        self.single_view = SingleCardView(lambda: self.state['current_collection'], self.save_card_change)
    ...
    async def save_card_change(self, api_card: ApiCard, set_code, rarity, language, quantity, condition, first_edition, image_id: Optional[int] = None, variant_id: Optional[str] = None):
        ...
        # target_variant_id is passed from SingleCardView logic now, but if None (shouldn't be), we try to recover
        if not variant_id:
             logger.warning("variant_id missing in save_card_change, this shouldn't happen with new logic.")
             return
    ...
    def open_single_view(self, card: ApiCard, is_owned: bool = False, quantity: int = 0, initial_set: str = None, owned_languages: Set[str] = None, rarity: str = None, set_name: str = None, language: str = None, condition: str = "Near Mint", first_edition: bool = False, image_url: str = None, image_id: int = None, set_price: float = 0.0):
        if self.state['view_scope'] == 'consolidated':
            ...
            self.single_view.render_consolidated_single_view(card, total_owned, owned_breakdown)

        elif self.state['view_scope'] == 'collectors':
             self.single_view.render_collectors_single_view(card, quantity, initial_set, rarity, set_name, language, condition, first_edition, image_url, image_id, set_price)
        else:
             # Fallback
             self.single_view.render_consolidated_single_view(card, 0, {})
