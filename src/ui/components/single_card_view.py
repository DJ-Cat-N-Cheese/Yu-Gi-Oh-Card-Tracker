...
class SingleCardView:
    def __init__(self, collection_provider: Callable[[], Optional[Collection]], save_callback: Callable):
        """
        collection_provider: Function that returns the current Collection object or None
        save_callback: Function to call when saving changes. Signature:
                       async def save_card_change(api_card, set_code, rarity, language, quantity, condition, first_edition, image_id, variant_id)
        """
        self.get_collection = collection_provider
    ...
    def setup_high_res_image_logic(self, img_id: int, high_res_remote_url: str, low_res_url: str, image_element: ui.image, current_id_check: Callable[[], bool] = None):
        ...
        if high_res_remote_url:
             async def download_task():
                 # Check if we are still looking at the same image (if check provided)
    ...
    def render_inventory_management(self, card: ApiCard, default_state: Dict, set_options: Dict[str, str], set_info_map: Dict, update_display_callback: Callable = None, dialog: ui.dialog = None):
        """
        ...
        with ui.card().classes('w-full bg-transparent p-4 gap-4'):
            with ui.row().classes('w-full gap-4'):
                ui.select(SUPPORTED_LANGUAGES, label='Language', value=input_state['language'],
                          on_change=lambda e: [input_state.update({'language': e.value}), update_display_callback() if update_display_callback else None]).classes('w-1/3')

                # Set Name dropdown
                ui.select(set_options, label='Set Name', value=input_state['set_base_code'],
                          on_change=lambda e: [input_state.update({'set_base_code': e.value}), update_display_callback() if update_display_callback else None]).classes('col-grow')

            with ui.row().classes('w-full gap-4'):
                ui.select(STANDARD_RARITIES, label='Rarity', value=input_state['rarity'],
                          on_change=lambda e: [input_state.update({'rarity': e.value}), update_display_callback() if update_display_callback else None]).classes('w-1/3')
                ui.select(['Mint', 'Near Mint', 'Played', 'Damaged'], label='Condition', value=input_state['condition'],
                          on_change=lambda e: [input_state.update({'condition': e.value}), update_display_callback() if update_display_callback else None]).classes('w-1/3')
                ui.checkbox('1st Edition', value=input_state['first_edition'],
                            on_change=lambda e: [input_state.update({'first_edition': e.value}), update_display_callback() if update_display_callback else None]).classes('my-auto')
            ...
            with ui.row().classes('w-full gap-4 items-center'):
                if card.card_images and len(card.card_images) > 1:
                    ...
                    # If current image_id is not in options (e.g. from API but not in images list?), default to first
                    ...
                    ui.select(art_options, label='Artwork', value=input_state['image_id'],
                              on_change=lambda e: [input_state.update({'image_id': e.value}), update_display_callback(image_update=True) if update_display_callback else None]).classes('col-grow')
            ...
            with ui.row().classes('w-full gap-4 justify-end q-mt-md'):
                async def handle_update(mode):
                    ...
                    # Check if this exact variant exists in API data
                    ...
                    if not variant_exists:
                        # If not in API sets, we might need to create it locally or it's a custom variant
                    ...
                    # Calculate current owned to handle ADD mode
                    current_owned = 0
            ...
            with ui.dialog().props('maximized transition-show=slide-up transition-hide=slide-down') as d, ui.card().classes('w-full h-full p-0 no-shadow'):
                ...
                with ui.row().classes('w-full h-full no-wrap gap-0'):
                    ...
                    with ui.column().classes('w-1/3 min-w-[300px] h-full bg-black items-center justify-center p-8 shrink-0'):
                        ...
                        # Store state for image updating
                        view_state = {'image_id': img_id}

                        def current_id_check():
                            return view_state['image_id'] == img_id

                        self.setup_high_res_image_logic(img_id, high_res_url, low_res_url, image_element, current_id_check)
                    ...
                    with ui.column().classes('col h-full bg-gray-900 text-white p-8 scroll-y-auto'):
                        ...
                        with inventory_expansion:
                            # Prepare Data for Inventory Management
                            set_options = {}
                            set_info_map = {}
                            if card.card_sets:
                                for s in card.card_sets:
                            ...
                            def update_image(image_update=False):
                                if image_update:
                                    new_img_id = inv_state['image_id']
                                    view_state['image_id'] = new_img_id
                                    ...
                                    self.setup_high_res_image_logic(
                                        ...
                                        low_res_url,
                                        image_element,
                                        current_id_check=lambda: view_state['image_id'] == new_img_id
                                    )
                            ...
                            self.render_inventory_management(
                                ...
                                set_options,
                                set_info_map,
                                update_display_callback=update_image,
                                dialog=d
                            )
            ...
            with ui.dialog().props('maximized transition-show=slide-up transition-hide=slide-down') as d, ui.card().classes('w-full h-full p-0 no-shadow'):
                ...
                with ui.row().classes('w-full h-full no-wrap gap-0'):
                    ...
                    with ui.column().classes('col h-full bg-gray-900 text-white p-8 scroll-y-auto'):
                        ...
                        def update_display_stats(image_update=False):
                            if image_update:
                                update_image()
                        ...
                        with inventory_expansion:
                             self.render_inventory_management(
                                 ...
                                 set_options,
                                 set_info_map,
                                 update_display_callback=update_display_stats,
                                 dialog=d
                             )
