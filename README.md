...
*   **Handles Massive Collections:** Web browsers can lag when trying to scroll through thousands of cards. OpenYuGi is designed to handle huge collections seamlessly. Searching, sorting, and filtering happen smoothly, even as your inventory grows.
...
The AI Scanner is OpenYuGi's most ambitious, complex, and partially experimental feature. It automates the digitization of physical cards using a multi-stage pipeline consisting of OpenCV Warp Perspective Correction, Optical Character Recognition (OCR), and Machine Learning Image Classification via YOLO.

Due to the highly unpredictable nature of physical cardboard—specifically the intense, light-scattering glare produced by (especially sleeved) Secret Rare, Ultimate Rare, and Starlight/Quarter Century Secret Rare foil patterns—the scanner has exceedingly strict environmental and operational requirements. If these requirements are not met, the error rate can be unacceptably high.
...
1.  **Lighting Environment (The Most Important Factor):** The lighting must be heavily diffused, even, and indirect. Direct overhead lighting, ring lights pointing directly down at the card, or flash photography can cause catastrophic glare on foil patterns. This glare can completely blind the OCR engines (turning text into white noise) and thoroughly confuse the YOLO feature matching models. Bouncing a desk lamp off a wall or using heavily frosted diffusers is advised. Most importantly make sure there is enough light!
2.  **Background Surface:** The card must be placed on a unicolor (ideally white), solid, non-reflective background (e.g., a sheet of printing paper or a clean cloth playmat). The initial stage of the pipeline relies on standard OpenCV edge-detection algorithms to find the four corners of the card. These algorithms require stark, immediate contrast between the physical card border and the background surface to accurately calculate the matrix required to digitally flatten (warp) the card image before OCR processing. If the backgroudn is white you can select the edge detection algorithm optimised for white backgrounds in the debug lab.
3.  **Hardware Mount:** A webcam should be mounted stably and parallel to the scanning surface to minimize extreme geometric angles that distort text aspect ratios.
...
*   **Preprocessing Modes:** Dictates how the raw video frame is manipulated before edge detection.
    *   *Classic:* Standard thresholding, grayscale conversion, and Canny edge detection. Best utilized for highly controlled, high-contrast setups. It is extremely fast computationally and works best.
    *   *YOLO / YOLO26 (experimental):* Bypasses traditional OpenCV edge detection and leverages the local `.pt` machine learning models to isolate the card bounding box regardless of contrast or background noise. It is slower computationally but sometimes more robust against complex backgrounds.
*   **Active OCR Track:** Determines which engine attempts to read the card's Set Code.
    *   *EasyOCR (not recommended):* A faster, lighter weight engine. While quick, it often struggles heavily with the highly stylized, tightly kerned, and sometimes compressed fonts utilized on Yu-Gi-Oh! cards, especially on highly reflective older printings.
    *   *DocTR (works best):* Slower and carries a much heavier Torch processing dependency, but provides vastly superior accuracy on complex text layouts, skewed angles, and microscopic set code parsing. Highly recommended.
*   **Art Style Match (YOLO):** Toggles the secondary identification pass. The system passes the cropped card art to a custom YOLO model which will compare it to the existing card images. This model attempts to determine the exact specific printing based on art similarity. Improves results. Remember to Index Images before use!
*   **Ambiguity Threshold:** A slider determining the confidence delta required before the scanner interrupts the automated batch flow. If the engine is unsure, it will halt and prompt the user via the Ambiguity Dialog to manually select the correct printing. Lowering this reduces the number of prompts to the user but can also increase error rate. Leave at default if unsure.
